...
router.get("/my-templates", auth, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error fetching your templates", error: error.message });
  }
});
...
// Create template - restricted to admins only; users should clone existing templates instead
router.post("/", auth, async (req, res) => {
  if (!req.user || req.user.role !== "admin") {
    return res
      ...
      .json({
        message:
          "Only admins can create new templates. Please clone an existing template instead.",
      });
  }
  try {
    const templateData = {
...
router.post("/:id/clone", auth, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error cloning template", error: error.message });
  }
});
...
router.post("/:id/rate", auth, async (req, res) => {
  try {
    ...
    template.rating =
      (template.rating * template.ratingCount + rating) /
...
router.get("/download/:id", auth, async (req, res) => {
  try {
    ...
    // Very simple HTML render from stored components
    const body = (template.components || [])
      .map((c) => {
        if (c.type === "text") return c.content || "";
        if (c.type === "image")
          return `<img src="${(c.attributes && c.attributes.src) || ""}" alt="" style="max-width:100%;"/>`;
        if (c.type === "button")
          return `<a href="${(c.attributes && c.attributes.href) || "#"}" style="display:inline-block;padding:12px 20px;background:#1e40af;color:#fff;border-radius:8px;text-decoration:none;">${c.content || "Button"}</a>`;
        if (c.type === "divider")
          return '<hr style="border:none;height:1px;background:#e5e7eb;"/>';
        if (c.type === "spacer")
          return `<div style="height:${c.styles && (c.styles.height || c.styles.padding || 16)}px"></div>`;
        return "";
      })
      .join("\n");
...
<body style="margin:0;padding:0;background:${template.settings?.backgroundColor || "#ffffff"};">
  <table role="presentation" cellpadding="0" cellspacing="0" width="100%"><tr><td align="center">
    <table role="presentation" cellpadding="0" cellspacing="0" width="${template.settings?.width || "600px"}" style="width:${template.settings?.width || "600px"};max-width:100%;font-family:${template.settings?.fontFamily || "Arial, sans-serif"};">
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="${(template.name || "template").replace(/[^a-z0-9-_]/gi, "_")}.html"`,
    );
    res.send(html);
  } catch (error) {
    ...
    res
      .status(500)
      .json({ message: "Error generating download", error: error.message });
  }
});
