...
const secretHex = process.env.ENCRYPTION_KEY;
const secretKey = secretHex ? Buffer.from(secretHex, 'hex') : crypto.randomBytes(32); // 32 bytes fallback for dev
if (!secretHex) {
  console.warn('ENCRYPTION_KEY is not set. Using a temporary key for development. Set ENCRYPTION_KEY for production.');
}
