...
const { checkFeatureAccess } = require('../middleware/trial');
...
router.post('/disconnect/:id', auth, checkFeatureAccess('whatsapp'), async (req, res) => {
  try {
...
router.delete('/accounts/:id', auth, checkFeatureAccess('whatsapp'), async (req, res) => {
  try {
    const account = await WhatsAppAccount.findOne({
      _id: req.params.id,
      user: req.user.id
...
router.get('/accounts', auth, checkFeatureAccess('whatsapp'), async (req, res) => {
  try {
...
router.get('/account-status/:id', auth, checkFeatureAccess('whatsapp'), async (req, res) => {
  try {
    const account = await WhatsAppAccount.findOne({
      _id: req.params.id,
      user: req.user.id
...
// @desc    Get QR code
router.get('/qr/:id', auth, checkFeatureAccess('whatsapp'), async (req, res) => {
  try {
    const account = await WhatsAppAccount.findOne({
      _id: req.params.id,
      user: req.user.id
...
router.post('/send', [auth, checkFeatureAccess('whatsapp'), upload.single('media')], async (req, res) => {
  try {
...
// @desc    Get campaigns
router.get('/campaigns', auth, checkFeatureAccess('whatsapp'), async (req, res) => {
  try {
...
router.post('/contacts/import', [auth, checkFeatureAccess('whatsapp'), upload.single('csvFile')], async (req, res) => {
  try {
    if (!req.file) {
...
router.get('/messages', auth, checkFeatureAccess('whatsapp'), async (req, res) => {
  try {
    const { 
      page = 1, 
      limit = 50, 
...
router.get('/analytics', auth, checkFeatureAccess('whatsapp'), async (req, res) => {
  try {
