...
// Start scraping job
router.post('/start', auth, checkFeatureAccess('scraper'), scraperLimiter, async (req, res) => {
  try {
...
router.get('/jobs', auth, checkFeatureAccess('scraper'), async (req, res) => {
  try {
...
router.get('/jobs/:jobId', auth, checkFeatureAccess('scraper'), async (req, res) => {
  try {
...
// Cancel job
router.post('/jobs/:jobId/cancel', auth, checkFeatureAccess('scraper'), async (req, res) => {
  try {
    const job = await Job.findOne({
      _id: req.params.jobId,
      user: req.user.id
...
router.get('/jobs/:jobId/results', auth, checkFeatureAccess('scraper'), async (req, res) => {
  try {
    const job = await Job.findOne({
      _id: req.params.jobId,
      user: req.user.id
