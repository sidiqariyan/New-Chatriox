...
if (process.env.NODE_ENV === "production") {
  ...
  try {
    httpsOptions = {
      key: fs.readFileSync(
        process.env.SSL_KEY_PATH || "/path/to/your/private-key.pem",
      ),
      cert: fs.readFileSync(
        process.env.SSL_CERT_PATH || "/path/to/your/certificate.pem",
      ),
  ...
  if (process.env.USE_HTTPS_DEV === "true") {
    try {
      ...
      server = https.createServer(httpsOptions, app);
      console.log(
        "HTTPS server configured for development with self-signed certificates",
      );
    } catch (error) {
      console.log(
        "Self-signed certificates not found, using HTTP for development",
      );
      server = http.createServer(app);
    }
  } else {
...
mongoose
  .connect(
    process.env.MONGODB_URI || "mongodb://localhost:27017/marketing_dashboard",
    {
...
try {
  ...
  console.error("Skipping /api/campaigns route:", e?.message || e);
}
...
const validateEmailContent = (req, res, next) => {
  ...
  if (htmlContent) {
    ...
    const hasSuspiciousContent =
      /<script|javascript:|vbscript:|onload|onerror/i.test(htmlContent);
    ...
    if (
      ...
      htmlContent.includes("position:sticky")
    ) {
      req.body.htmlContent = htmlContent.replace(
        /(position:\s*(fixed|sticky))/gi,
        "position:relative",
      );
    }
  }
...
const aiGenerationLimiter = rateLimit({
  ...
  message: {
    success: false,
    message:
      "AI generation limit exceeded. Please try again later or upgrade your plan.",
  },
...
server.listen(actualPort, () => {
  ...
  console.log(
    `MongoDB URI: ${process.env.MONGODB_URI || "mongodb://localhost:27017/marketing_dashboard"}`,
  );
});
