...
class ApiService {
  ...
  async getStats(timeRange = "7d") {
    const response = await this.request(
      `/dashboard/stats?timeRange=${timeRange}`,
    );
    return response.data || response;
  }
  ...
  async getMyTemplates(page = 1, limit = 20) {
    const response = await this.request(
      `/templates/my-templates?page=${page}&limit=${limit}`,
    );
    return response.data || response;
  }
  ...
  async createTicket(
    ...
    const response = await this.request("/help/tickets", {
      method: "POST",
  ...
  async listCaseStudies(all = false) {
    const response = await this.request(`/case-studies${all ? "?all=1" : ""}`);
    return response.data || response;
  }
  async getCaseStudy(slug: string) {
    const response = await this.request(`/case-studies/${slug}`);
    return response.data || response;
  }
  async createCaseStudy(data: any) {
    const response = await this.request(`/case-studies`, {
      method: "POST",
      body: JSON.stringify(data),
    });
    return response.data || response;
  ...
  async updateCaseStudy(id: string, data: any) {
    const response = await this.request(`/case-studies/${id}`, {
      method: "PUT",
      body: JSON.stringify(data),
    });
    return response.data || response;
...
export const caseStudiesApi = {
  list: (all = false) => apiService.listCaseStudies(all),
  get: (slug: string) => apiService.getCaseStudy(slug),
  create: (data: any) => apiService.createCaseStudy(data),
  update: (id: string, data: any) => apiService.updateCaseStudy(id, data),
  remove: (id: string) => apiService.deleteCaseStudy(id),
};
