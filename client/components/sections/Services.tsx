        ...
        {services.map((s, i) => (
          <div key={s.key} className={`grid md:grid-cols-2 gap-6 items-stretch ${i % 2 ? 'md:[&>div:first-child]:order-2' : ''}`}>
            <div className="rounded-2xl glass p-6">
              ...
              <div className="mt-5">
                <a href={(() => { const map: Record<string, string> = { email: '/bulk-email', whatsapp: '/whatsapp', validation: '/validation', Lead: '/mail-scraper', ai: '/ai-analyzer', templates: '/templates' }; return map[s.key] || '/services'; })()} className="px-4 py-2 rounded-md text-white font-semibold bg-sky-500 hover:bg-sky-700">Start {s.title}</a>
              </div>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/5 p-6 flex items-center justify-center overflow-hidden">
              {s.image ? (
                ...
                <div className="w-full h-48 md:h-64 rounded-xl bg-sky-500/10 flex items-center justify-center">
